...
const startServer = (WEB_CONFIG, UI_CONFIG)=>{
  const {
    PORT = 8080,
    HOST = 'localhost',
    WEB_ROOT = 'web/site/',
  ...
  server.ext('onPreResponse', function (request, reply) {
    if (request.response.isBoom && request.response.output && (request.response.output.statusCode===404)) {
      ...
      if(isIndexRoute){
        ...
        if(args.hot){
          const page = server.plugins.webpack.devMiddleware.fileSystem.readFileSync(`${webRoot}/index.html`);
...
const init = ()=>{
  ...
  startServer(WEB_CONFIG, UI_CONFIG);
};

config.on('ready', init);
